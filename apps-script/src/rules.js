/**
 * Booking rules shared by the Apps Script web app and the Node tests.
 * Pure functions only. Dates are 'YYYY-MM-DD' strings in Israel local time; timed
 * events use local 'YYYY-MM-DDTHH:mm' strings. A "night" is named by the date it starts.
 */

var CHECK_IN_MINUTES = 15 * 60;
var CHECK_OUT_MINUTES = 11 * 60;
var MAX_NIGHTS = 14;
var HORIZON_DAYS = 365;
var HOLD_HOURS = 24;
var MAX_ADULTS = 3;
var NOTES_MAX = 450;
...
function stayRange(stayType, checkIn, checkOut) {
 ...
 */
function blockedNights(events, from, to) {
...
function conflictingNights(blocked, start, end) {
  var set = {};
...
function estimatePrice(stayType, nights, adults) {
...
function isHoldActive(createdAtIso, nowMs) {
...
/** Validates a website booking request. `today` is the Israel-local date. */
